...
public class UsuarioController {
    ...
    public String guardarUsuario(@Valid @ModelAttribute("usuario") Usuario usuario, BindingResult result, @RequestParam(value = "file") MultipartFile foto, Model model, RedirectAttributes flash, SessionStatus sessionStatus) {

        List<Puesto> puesto = puestoServicio.findAll();

        boolean isNuevoUsuario = (usuario.getIdUsuario() == null);

        if (isNuevoUsuario || (!usuario.getContrasena().isEmpty() && !usuario.getContrasena().equals(usuario.getConfirmarContrasena()))) {
            result.rejectValue("confirmarContrasena", "error.confirmarContrasena", "Las contraseñas no coinciden.");
        }

        if (isNuevoUsuario && usuarioServicio.existsByNombreUsuario(usuario.getNombreUsuario())) {
            result.rejectValue("nombreUsuario", "error.nombreUsuario", "El nombre de usuario ya está en uso.");
        }

        if (result.hasErrors() || (usuario.getPuesto() == null || usuario.getPuesto().getIdPuesto() == null)) {
            if (usuario.getPuesto() == null || usuario.getPuesto().getIdPuesto() == null) {
                result.rejectValue("puesto", "error.puesto", "Selecciona un puesto válido.");
            }
            model.addAttribute("usuario", usuario);
            model.addAttribute("listaPuestos", puesto);
            return "admin/crear";
